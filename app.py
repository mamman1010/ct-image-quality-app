...
input_method = st.radio("📌 Select input method:", ["Abdominal Volume", "Abdominal Circumference"], help="Choose whether to enter abdominal volume in l or circumference in cm")
...
if input_method == "Abdominal Volume":
    volume = st.number_input("📏 Enter abdominal volume (rounded, in cm³):", min_value=0, step=1, help="This value should be derived from 3D Slicer segmentation between -50 and 200 HU")
    volume_int = int(round(volume))
    filtered = df[(df['volume'] == volume_int) & (df['kVp'] == kVp)]
...
# Use ML Prediction
if input_method == "Abdominal Volume":
    input_features = np.array([[volume_int, kVp]], dtype=np.float32)
...
x_val = volume_int if input_method == "Abdominal Volume" else ab_circ_int
